...
public class HistoryController {
    ...
    public List<HistoryResponseDTO> readAll(@PathVariable("date") String strDate,
        ...
        if (strDate != null) {
            try {
                LocalDate.parse(strDate);  // Validate date format
            } catch (DateTimeParseException e) {
                throw new FitBuddyException(DATE_NOT_VALID);  // Handle invalid date format
            }
            Integer userId = appUserPrincipal.getId();
            if (userId != null) {
        ...
        return Collections.emptyList();  // Return empty list if no valid date provided
    }
    ...
    public void update(@PathVariable("id") @NotNull Integer historyId,
        ...
        if (userId != null) {
            ...
            if (historyResponseDTO != null && historyResponseDTO.getAppUserId().equals(userId)) {
                ...
                throw new FitBuddyException("UserIds don't match. Cannot update others' history.");
            }
        }
    }
    ...
    public void delete(@PathVariable("id") @NotNull Integer historyId,
        ...
        if (userId != null) {
            ...
            if (historyResponseDTO != null && historyResponseDTO.getAppUserId().equals(userId)) {
                ...
                throw new FitBuddyException("UserIds don't match. Cannot delete others' history.");
            }
        }
    }
}
